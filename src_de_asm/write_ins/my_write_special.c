...
** Last update Thu Apr  3 15:54:19 2014 Antoine Plaskowski
*/
...
static int	my_help_write(header_t *header, char *champ, int i, int fd)
{
  if (my_putchar('%', fd))
    return (-1);
  return (my_write_indirect(header, champ, i, fd));
}
...
static int	my_write(header_t *header, char *champ, int i, int fd)
{
  ...
  while (j < MAX_ARGS_NUMBER && tab[j] != 0)
	...
	status = my_help_write(header, champ, i, fd);
      if (status < 1)
	return (-1);
      i += status;
      j++;
